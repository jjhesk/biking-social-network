 ...
 */
function info_expert(html) {
	var exp, n = html.length;
	if (n > 90) {
		exp = html.substring(0, 60);
	} else {
		exp = html;
	}
...
function jps_shortString(str, limit) {
	...
	var nwords = body.match(r).length;
...
function headcss(path) {
	...
	var d = urlPathArray[urlPathArray.length - 1].split('.');
	d.pop();
	var l = d.length, e;
	if (l >= 2) {
		e = d[1] + "_css";
