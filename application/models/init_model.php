...
class Init_model extends CI_Model 
{
	...
	public function array_inner_join_in_same_key($data, $merge_in_second_multiple_array_key=''){
			 ...
			 * ....
			 * The $merge_in_second_multiple_array_key is not the key for merging, 
			 * it just open the new array key in the first main array to carry multiple second arrays.
			 * 
			 * */
				$result=array();
				$temp_result=array();
					...
					for($record1=0;$record1<count($data['array'][0]);$record1++){
						...
						for($table2=1;$table2<count($data['array']);$table2++){
							...
							for($record2=0;$record2<count($data['array'][$table2]);$record2++){
								...
								foreach($data['array'][$table2][$record2] as $key2=>$val2){
									foreach($data['array'][0][$record1] as $key1=>$val1){
										if( ($key2==$key1)&&($key2!=$data['condition_key'][$table2]) ){
											unset($data['array'][$table2][$record2][$key2]);
										}
									}
								}
								
								if($data['array'][0][$record1][$data['condition_key'][0]]==$data['array'][$table2][$record2][$data['condition_key'][$table2]]){
									if($merge_in_second_multiple_array_key==''){
										$temp_result=array_merge($temp_result, $data['array'][$table2][$record2]);
										$condition_counter++;
									}else{
										if( (isset($temp_result[$merge_in_second_multiple_array_key])) ){
											//array_push($temp_result[$merge_in_second_multiple_array_key], $data['array'][$table2][$record2]);
											//echo "<br/>init model 120 ".$merge_in_second_multiple_array_key.":".count($temp_result[$merge_in_second_multiple_array_key]);
										}else{
											$temp_result[$merge_in_second_multiple_array_key]=array();
										}
										array_push($temp_result[$merge_in_second_multiple_array_key], $data['array'][$table2][$record2]);
										//echo "<br/>init model 120 ".$record1.":".count($temp_result[$merge_in_second_multiple_array_key]);
										//print_r($temp_result[$merge_in_second_multiple_array_key]);
						...
						if($condition_counter>=count($data['array'])){
							//echo "<br/>init model 115 ";
							//print_r($temp_result);
							$result[$counter]=$temp_result;
							$counter++;
						}
					}
					//echo "<br/>init model 145 ".count($result);
					return $result;
				}else{
	...
	public function array_outer_join_in_same_key($data, $merge_in_second_multiple_array_key=''){
				//$result=array();
				$temp_result=array();
				$counter=0;
				//echo "<br/>init_model 43 ".count($data['array']);
				...
				if(count($data['array'])>=2){
					...
					for($record1=0;$record1<count($data['array'][0]);$record1++){
						...
						if(!isset($temp_result[$merge_in_second_multiple_array_key])){
							$temp_result[$merge_in_second_multiple_array_key]="";
						}
						$result[$counter]=$temp_result;
						$counter++;
						$temp_result=null;
	...
	public function bll_get_header($is_logined=true, $profile_tabs_html=''){
			...
			$this->load->model("ctrl_login_model", "login_model");
