...
class Friends_model extends CI_Model {
	...
	public function get_user_of_friends_details($id){
	    $list_ids_of_this_person = $this->myfriends($id)-> result_array();
	    //$list_ids_of_this_person=$list_ids_of_this_person[0];
        $dump=array();
        foreach($list_ids_of_this_person as $key=>$value){
           $id=$value["object_user_id"];
          $dump[]=$this->get_profile_detail($id);
        }
        return $dump;
	}
	public function myfriends_package() {
		/*
		 * 
		...
		*/
		...
		$myid = $this -> me_id();
        return $this->get_friend_detail($myid);
	}
    private function get_profile_detail($myid){
       // $data_row=array();
        $data_row=$this->get_user_row($myid);
       // print($data_row);
       // count($data_row);
       // die();
        $mename = $data_row['firstname'].' '.$data_row['lastname'];
        if($data_row['profile_image']==NULL){
            $profile_pic=$data["base"] = base_url() . 'images/common/no-photo.png';
        }else $profile_pic=$data_row['profile_image'];
        ...
        'location' => $data_row['country'],
        //current job
        'work' => $data_row['career'],
        //the time stamp in raw data of the person to be updated the most recently
        'lastupdate' => $data_row['last_updated'],
