...
class Activity_model extends CI_Model {
	...
	public function set_activity_privacy($data){
		 ...
		 */
		$privacy=$this->DBget_privacy($data['app_id'], $data['userinfo']);
	...
	public function DBget_get_open_graph_object($activity_id){
		...
		if($activity!=null){
			...
			//print_r($image);
			if($image==null){
				$image="/images/colorbox/fansliving_32x32.png";
			}else{
			...
			$description=$this->get_description_from_coordinates($activity, $app_data['product_name'], $userinfo['nickname'], $activity['coordinates_json'], false);
		
			$result['open_graph_object_data']=array(
				'self_url'=>site_url()."/portal/open_graph_object/".$activity_id,
				'title'=>$activity['title'],
				'image'=>$image,
				'description'=>$description
			);
			$result['returnurl']=site_url()."/profile/activity/".$activity_id;
			return $result;
	...
	public function create_new_activity($user_id, $app_id, $password) {
		...
		$data = array(
			...
			'max_heart_rate' => '60',
			'total_calories' => '0',
			'privacy'=>$this->DBget_privacy($app_id, $userinfo),
	...
	public function DBset_new_community($app_id, $activity_id, $user_inhouse_id, $title){
		//DAL
				$query = $this -> db -> get_where('app_page_layout', array('app_id' => $app_id));
				$getdata = $query -> result_array();
				$appname = $getdata[0]['tab_label'];
				$post_data = array(
				 'news_type' => "activity_thread",
				 'source_id' => $activity_id,
				 'comment_user_id' => $user_inhouse_id,
				 //'comment_text' => 'I started an activity near '.$title,
				 'comment_text' => $title,
				 );
				 
				if ($title == 'Activity' && $appname == 'PetNfans'){
					$post_data['comment_text'] = 'I just uploaded a photo with '.$appname;
				}elseif($title == 'Activity' && $appname != 'PetNfans'){
					$post_data['comment_text'] = 'I started an activity with '.$appname;
				}
			...
			foreach($permission as $_permission){
				if($_permission=="facebook"){
					...
					if($facebook_oauth!=null){
						//$new_data['activity_data_id']=$activity_id;
						//$new_data['token']=$facebook_oauth;
						//$this->facebook_model->upload_facebook_activity($new_data);
						//echo "<br/>api 161 after upload ".$facebook_oauth['code'];
						$facebook_oauth=$this->facebook_model->FBget_access_token_by_code($facebook_oauth);
						if(isset($data['activity_id'])){
							echo $this->facebook_model->upload_facebook_open_graph_object_by_activity($data['activity_id'], $facebook_oauth);
						}
					}	
	...
	public function DBget_app_name_by_app_id($app_id){
		if ($app_id!=0) {
	...
	public function DBget_device_found_by_app_users_data_id($app_users_data_id){
		$this->db->where("app_users_data_id", $app_users_data_id);
		$this->db->where("mode", "pet_found");
		$this->db->order_by("last_updated", "desc");
		$result=$this->db->get("activity_data");
		$result=$result->result_array();
	...
	public function activity_manual_post($post=null){
		...
		if(count($userinfo)>0){
			...
			if ($activity_id==null) {
				...
				// Post to community
				$this->activity_model->DBset_new_community($_POST['app_id'], $activity_id,$_POST['user_inhouse_id'], $new_data['title']);
				...
				/*if ($post['mode']=="auto") {
					//echo "<br/>api 134";
					$kml_path = '<?xml version="1.0" encoding="UTF-8"?><kml xmlns="http://earth.google.com/kml/2.1"><Document><Placemark id="path"><LineString><extrude>1</extrude><altitudeMode>relativeToGround</altitudeMode><coordinates>114.17325,22.28297,0 114.17324,22.28296,0 </coordinates></LineString></Placemark><Placemark id="starting_point"><Point><coordinates>114.17324,22.28296</coordinates></Point></Placemark></Document></kml>';
					$this->db->set('kml_path', $kml_path);
					$this->db->set('kml_checkpoints', $kml_path);
					$this->db->where('activity_id', $activity_id);
					$this->db->update('activity_data'); 
				} else {
					// Generate kml contents for maps
					//echo "<br/>api 142";
					$this->googlemaps_model->update_path_kml($activity_id);
					$this->googlemaps_model->update_checkpoint_kml($activity_id);
				}*/
				if( (isset($_POST['coordinate_json']))&&($_POST['coordinate_json']!='[]') ){
					$this->googlemaps_model->update_path_kml($activity_id);
					$this->googlemaps_model->update_checkpoint_kml($activity_id);
				}
			// Else update row
			/*} else {
				//echo "<br/>api 148";
				// Update node in previous activity
				$new_data['prev_activity_id'] = $this->activity_model->get_user_single_record($activity_id, 'prev_activity_id');
			
				// Update row
				$this->db->where('activity_id', $activity_id);
				$this->db->update('activity_data', $new_data); 
			}*/
			//echo "<br/>activity model  1104";
			// Give response
	        //$this->response(array('status' => 'success', 'activity_id' => $activity_id), 200);  
	...
	public function DBget_apps_user($app_id, $userinfo){
		//DAL
		$this->init_model->sensitive_session[]="DBget_apps_user($app_id)";
		if($this->native_session->get("DBget_apps_user($app_id)")==null){
			$this->db->where("app_id", $app_id);
			$this->db->where("user_id", $userinfo['user_inhouse_id']);
			$result=$this->db->get("app_users");
			$result=$result->result_array();
			if(count($result)>0){
				$this->native_session->set("DBget_apps_user($app_id)", $result[0]);
				return $result[0];
			}else{
				return null;
			}
		}else{
			return $this->native_session->get("DBget_apps_user($app_id)");
		}
	}
	public function DBget_privacy($app_id, $userinfo){
		//BLL
		$this->init_model->sensitive_session[]="DBget_privacy($app_id)";
		if($this->native_session->get("DBget_privacy($app_id)")==null){
			$result=$this->DBget_apps_user($app_id, $userinfo);
			if($result!=null){
				//apps_user have privacy seetting 
				$this->native_session->set("DBget_privacy($app_id)", $result['default_privacy']);
				return $result['default_privacy'];
			}else{
				//apps_user does not have privacy setting,
				//so use user_inhouse_data as default privacy setting
				//$this->native_session->set("DBget_privacy($app_id, $userinfo)", $userinfo['default_privacy']);
				//$result=$userinfo['default_privacy'];
				//return $result;
				$this->native_session->set("DBget_privacy($app_id)", "default");
				$result="default";
				return $result;
			}
		}else{
			return $this->native_session->get("DBget_privacy($app_id)");
		}
	}
	...
	public function bll_activity_manual_post_1($post, $userinfo, $last_activity_id){
			...
			//print_r($post);
			$apps_info=$this->DBget_apps_data_by_apps_id($post['app_id']);
			if($apps_info!=null)
				$apps_name=$apps_info['product_name'];
			else
				$apps_name="Fansliving";
			$title=$apps_name;
			...
			/*	
			$coordinates = json_decode($this->input->post('coordinates_json'), TRUE);
			$this->load->model("googlemaps_model");
			$location=$this->googlemaps_model->get_location_by_latitude_longitude_from_google($coordinates);
			$description=$userinfo['nickname']." had create a new ".$apps_name." record";
			if($location!=null){
				$description.=" in ".$location;
			}
			$description.=" at ".date("F j, Y").". You can click here to review his story.";
			*/
			if( (!isset($post['description']))||($post['description']==null) ){
				$description=$this->get_description_from_coordinates(null, $apps_name, $userinfo['nickname'], $post['coordinates_json']);
			}else {
				$description=$post['description'];
			}	
			
			/*if($post['coordinates_json']!=""){
				$coordinate=json_decode($post['coordinates_json'], true);
				echo "<br/>activity model 1439 ";
				print_r($coordinate);
			}*/
			
			$privacy=$this->DBget_privacy($post['app_id'], $userinfo);
			//echo "<br/>api 72";
			// Retrieve data
			$data=$post;
