<?
	...
	echo get_content_list_table($winner_arr,$campaign);
	
	
	
	function get_content_list_table ($winner_arr,$campaign) {
		...
		<th width=50px >facebook</th><th width=50px >score</th>
		<th width=50px >NOs.</th><th width=100px >Upload Date</th><th width=50px >publish</th><th width=150px >Action</th></tr>";
		
		$id_arr=array();
		if (is_array($winner_arr)) {
			//$ui.="<form action='cms_action.php' method='POST' >";
		//	$ui.="<input type=hidden name=product_action value='update_catalog' >";
			foreach ($winner_arr as $row) {
		...
		//		$ui.="<div id='".$content_id."_row' >";
				...
				$ui.="<td width=50px >$score</td>";
				$ui.="<td width=50px ><input type='text' class='text' name='".$cid."_field' id='".$cid."_field' value='$nos'  style='width:30px' maxlength=3 onkeyup='javascript:update_nos($cid, this.value, event );'  > </b></a></td>";
				...
				$ui.="<td  width=150px >  [ <a href='mailto:$user_email'>email</a> ]  [ <a href='javascript:javascript:update_winner($cid,-1)'>remove</a> ]</td>";
				$ui.="</tr>";
