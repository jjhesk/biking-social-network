...
var now_album="album1";
function openPopup(id) {
	
	//obj=dojo.byId(id);
	obj=document.getElementById(id);
	var album=obj.getAttribute('filePath');
	var fileType=obj.getAttribute('fileType')
	
	//alert(filePath + " || " +fileType);
	if (fileType=='photo') {
		//return hs.expand(obj, { src: filePath,dimmingOpacity: 0.7,align : 'center'});
		$("#"+now_album).fadeOut('300',function () { 
		...
		var videoFilePath='viewVideo.php?fileType='+fileType+'&filePath='+filePath;
		return hs.htmlExpand(obj, { objectType: 'iframe',src:videoFilePath,align:'center',minWidth:350, width:350,minHeight:400,height:400, preserveContent:false,objectLoadTime:'after' });
	}
...
//						alert(imageId);
						openPopup(imageId)
//						return hs.expand(this, { src: this.getAttribute('longdesc'),dimmingOpacity: 0.7, align : 'center' }); 
...
function refresh(onload)
{
	...
	images_top = imageflow_div.offsetTop+verticalOffset;
	...
	caption_div.style.marginTop = (images_width * 0.03 +verticalOffset) + 'px';

	/* Change scrollbar div properties */
	scrollbar_div.style.marginTop = ((images_width +verticalOffset )* 0.02 ) + 'px';
