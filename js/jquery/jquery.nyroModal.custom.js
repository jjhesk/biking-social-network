		...
		_animations = {
			basic: {
				showBg: function(nm, clb) {
					nm.elts.bg.css({opacity: 0.5}).show();
					clb();
				},
	...
	$.nmFilters({
		link: {
			is: function(nm) {
				...
				//var reh = nm.opener.is('[urlhesk]');
				if (ret) nm.store.link = nm.getInternal()._extractUrl(nm.opener.attr('href'));
				//if (reh) nm.store.link = nm.getInternal()._extractUrl(nm.opener.attr('urlhesk'));
				return ret;
			},
			init: function(nm) {
				nm.loadFilter = 'link';
